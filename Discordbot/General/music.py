...
        self.inactivity_tasks = {}  # To track inactivity check tasks per guild
...
                await self.stop_inactivity_check(ctx.guild.id)
...
        # Cancel any existing inactivity task before starting a new one
        if guild_id in self.inactivity_tasks:
            self.inactivity_tasks[guild_id].cancel()

        # Start a new inactivity check task
        self.inactivity_tasks[guild_id] = asyncio.create_task(self.check_inactivity(guild_id))

    async def stop_inactivity_check(self, guild_id):
        # Cancel and remove the inactivity task if it exists
        if guild_id in self.inactivity_tasks:
            self.inactivity_tasks[guild_id].cancel()
